...
@pytest.mark.parametrize(
    "variable, year, month, nvm, cell_method, statistic",
    [
    ...
    expected = [
        {
            ...
            "network_variable_name": f"{history.station.network.name} {nvm}",
            "cell_method": cell_method,
            "statistic": statistic,
            "data_coverage": pytest.approx(1.0),
        }
        for history in tst_histories
        if history.station.publish and history.station.network.publish
    ]
    assert len(result) == len(expected)
    assert sorted(result, key=lambda r: r["station_name"]) == sorted(
        result, key=lambda r: r["station_name"]
    )
