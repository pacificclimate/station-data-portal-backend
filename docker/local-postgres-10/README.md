...
```
docker pull pcic/sdpb-local-postgres-10
```
...
```
py3clean .
docker run -it -v $(pwd):/codebase pcic/sdpb-local-postgres-10
```
...
```
docker build -t pcic/sdpb-local-postgres-10 -f docker/local-postgres-10/Dockerfile --no-cache .
```
